...
CREATE INDEX chair_access_token_index ON chairs (access_token);
CREATE INDEX chair_is_active_index ON chairs (is_active);
...
CREATE INDEX rides_user_id_index ON rides (user_id, evaluation);
...
(
  ...
  PRIMARY KEY (ride_id, status, created_at)
)
...
(
  ...
  COMMENT 'クーポンテーブル';
CREATE INDEX coupons_used_by_index ON coupons (used_by);
