...
(
  ...
  COMMENT = 'ライドステータスの最新テーブル';

INSERT INTO ride_status SELECT t1.ride_id, t1.status, t1.created_at
FROM  ride_statuses t1
JOIN (
    SELECT ride_id, MAX(created_at) AS latest_created_at
    FROM ride_statuses
    GROUP BY ride_id
) t2
ON t1.ride_id = t2.ride_id AND t1.created_at = t2.latest_created_at;
